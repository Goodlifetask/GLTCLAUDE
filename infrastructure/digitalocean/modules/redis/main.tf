...
variable "size"          { type = string  default = "db-s-1vcpu-1gb" }
variable "redis_version" { type = string  default = "7" }
...
resource "digitalocean_database_firewall" "redis" {
  ...
  rule {
    type  = "ip_addr"
    value = "0.0.0.0/0"   # Tighten to VPC CIDR after first deploy
  }
}
...
output "password"  { value = digitalocean_database_cluster.redis.password  sensitive = true }
output "redis_url" {
