...
export default function FamilySetupPage() {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const res = await api.family.create({ name: familyName });
      if (inviteEmail.trim()) {
        await api.family.invite({ email: inviteEmail.trim() });
      }
      return res;
    },
    onSuccess: () => {
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--color-text-muted)', margin: 0, lineHeight: 1.6 }}>
            Create a shared space where your family can assign tasks, reminders and events to each other.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          >
            {mutation.isPending ? 'Creating family...' : 'Create Family'}
          </button>
        </form>
