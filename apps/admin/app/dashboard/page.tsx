...
function CategoriesPage({ showToast }: { showToast: (msg: string, type?: 'success' | 'error') => void }) {
  ...
  async function generateIcon(name: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.icon) {
          setNewEmoji(data.icon);
          setIconGenerated(true);
        }
      }
    } catch {
      // silently fall back to default emoji
    } finally {
  ...
  function handleNameChange(value: string) {
    ...
    if (value.trim().length >= 2) {
      iconDebounceRef.current = setTimeout(() => generateIcon(value), 600);
    } else {
      setNewEmoji('');
    }
  }
            ...
            <div className="card-body">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                <div style={{
                  ...
                  {iconLoading ? (
                    <span style={{ fontSize: 14, color: 'var(--t4)', animation: 'spin 1s linear infinite' }}>⟳</span>
                  ) : (
                    newEmoji || '📁'
                  )}
                  {iconGenerated && !iconLoading && (
                    <span style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, color: 'var(--t3)', marginBottom: 3 }}>
                    {iconLoading
                      ? '✨ Claude is picking the best icon…'
                      : iconGenerated
                      ? '✨ Icon generated by Claude — you can regenerate below'
                      : newName.length >= 2
                      ? 'Type at least 2 characters to auto-generate an icon'
                      : 'Icon preview'}
                  </div>
                  <button
                    style={{
                      ...
                      opacity: iconLoading || newName.trim().length < 2 || addLoading ? 0.5 : 1,
                    }}
                    onClick={() => generateIcon(newName)}
                    disabled={iconLoading || newName.trim().length < 2 || addLoading}
                  >
                    {iconLoading ? '…' : '✨ Regenerate'}
                  </button>
                </div>
              </div>
